...
bool CSHA1::SHA_GO(const void *lpData_Input, const INT64 nDataSize, char *lpSHACode_Output)
{
	...
	INT64 nInputLen = nDataSize;
	INT64 nDealDataLen = 0;							//	the length of can-deal-data, this times;
	for(INT64 pos=0 ; pos<=nInputLen ; pos+=64)
	{
		if (nInputLen - pos >= 64)
		{
			nDealDataLen = 64;						// input-data is enough fill 64bytes,
			memset(Message_Block, 0, sizeof(Message_Block));
			memcpy(Message_Block, (CHAR*)lpData_Input + pos, nDealDataLen);
...
void CSHA1::AddDataLen(INT64 nDealDataLen)
{
