...
class CSHA1
{
	public:
		CSHA1();
		virtual ~CSHA1();
		bool SHA_GO(const void *lpData_Input, const INT64 nDataSize, char *lpSHACode_Output);
	private:
		...
		INT64 Message_Block_Index;				// Index into message block array
	private:
		void SHAInit();
		void AddDataLen(INT64 nDealDataLen);
